      ...
      <div class="cart_content "  >
         <ul class="cart_list" style="line-height: 50px; font-size: 20px;">
            ...
            <input type="checkbox" name="check" id="check" checked="checked"
              ...
              <label > 주문번호:</label>
               <span name="cart_id" id="cart_id<%=cart_id%>" data-P_id="<%=p_id %>" ><%=cart_id%></span><br>
               <label > 상품명:</label> <a
 	...
 	//숫자 천단위 ,찍기
		...
		$("#order").on("click", function() {
			...
			$("input[name=check]:checked").each(function (index, data) {
				var cart_id = $(this).val();
				p_id = $("#cart_id"+cart_id).attr("data-P_id");
				p_amount = $("#cartAmount"+cart_id).val();
				console.log(cart_id+" "+p_id+" "+p_amount);
				id_arr.push(p_id+","+p_amount);
				amount_arr.push({"p_amount":p_amount});
			});//end each
			console.log(id_arr);
			$.ajax({
				type: "post",
				url : "OrderServlet_TEST2",
				////////Content-Type 명시함
